...
function nuevaPartida() {
    ...
    paises = getPaisesRnd(gameData.countries);
...
function finalizarPartida() {
    ...
    //actualizar grafico lineal
    dataIntentos.addRows([
        [intentos, parseInt(tiempoIntento,10)]
    ]);
...
function actualizarTimer() {
    let tiempoRestante = document.getElementById("timer").innerHTML;
...
// marcador inicial del instituto
var marker = L.marker([28.456046923017453, -16.28343672568356]).addTo(mymap);
marker.bindPopup("<b>Bienvenido al CIFP César Manrique!</b>").openPopup();
...
function drawChart() {
    ...
    chartIntentos = new google.charts.Line(document.getElementById('chartIntentos'));
...
function activarCiudadPais(paises) {
    ...
    paises.forEach(({pais,ciudad}) => {
        ...
        var drop = document.importNode(countryTemplate.content, true).firstElementChild;
        drop.firstElementChild.innerHTML = pais.name;
        drop.setAttribute("class", "country droppable");
        drop.setAttribute("data-code", pais.code);
        drop.setAttribute("data-name", pais.name);
        ...
        $(drop).droppable({
            ...
            drop: function (event, ui) {
                ...
                if (ui.draggable[0].dataset.code == this.dataset.code) {
                    ...
                    //Viajar en el mapa
                    let latlon = ui.draggable[0].dataset.location.split(",");
                    mymap.flyTo([latlon[0], latlon[1]], 13);
                    ...
                    $(ui.draggable[0]).draggable({  //stop drag revert
                         revert: false
                     })
                    ...
                    ocurrencias.find(pais => {
                        if (pais.pais == drop.dataset.name){
                            pais.ocurrencias += 1;
                        }
                        dataOcurrencias.addRows([
                            [pais.pais, pais.ocurrencias]
                        ])
                    });
...
function appendCiudadesPaises(){
    ...
    while(cities.length > 0){
        ...
        let indice = Math.floor(Math.random() * cities.length);
        let ciudad = cities[indice];
    ...
    while(countries.length > 0){
         ...
         let indice = Math.floor(Math.random() * countries.length);
         let pais = countries[indice];
         countriesContainer.appendChild(pais);
         countries.splice(indice, 1);
    }
}
